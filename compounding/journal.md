...
## 2026-02-09: AI 图像生成的正确定位

- **发生了什么**: 最初规划了 6 个 AI 图像生成场景（星座地图、身份演化、状态调频、饮食场景、进度路径、章节庆祝），发现 5/6 是数据可视化任务——前端渲染在交互性、一致性、速度、成本上全面优于 AI 生成。
- **原因**: 将"能用 AI 生成"等同于"应该用 AI 生成"。数据可视化有确定性输入和结构化输出，不需要 AI 的创造性和上下文理解能力。
- **下次怎么做**: AI 图像生成只在需要高度个性化、上下文相关、无法预制的场景使用。正确定位是教练干预工具（未来自我可视化、隐喻图像化、场景预演、认知重构），不是数据展示。判断标准："预制 10 张图能覆盖吗？"如果能，就不用 AI 生成。
