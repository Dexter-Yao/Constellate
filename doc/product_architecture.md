...
全局提示词管理中心。DeepAgent 没有内置的统一提示词管理，需自建。

```python
# aligner/config/prompts.py
...
```
prompts/
├── coach_system.md       # Coach Agent 主系统提示词
└── ...                   # 后续按需添加
```

设计原则：
- 提示词以 Markdown 文件存储，git diff 友好
- 每个文件是完整提示词，不做模板变量替换
...
|----------|---------|--------|----------|
| `/user/{id}/` | StoreBackend | 跨 session 持久 | profile/, ledger/, derived/, coach/ |
